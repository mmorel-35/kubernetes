...
type merger interface {
	...
	Merge(source string, update any) error
}
...
type mux struct {
	...
	sources map[string]chan any
}

// newMux creates a new mux that can merge changes from multiple sources.
func newMux(merger merger) *mux {
	mux := &mux{
		sources: make(map[string]chan any),
		merger:  merger,
	}
	return mux
}
...
// union.
func (m *mux) ChannelWithContext(ctx context.Context, source string) chan any {
	if len(source) == 0 {
...
func (m *mux) listen(source string, listenChannel <-chan any) {
