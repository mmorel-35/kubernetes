...
// Numbers decoded into any fields are converted to int64 or float64.
...
// Values which are map[string]any or []any are recursively visited
func ConvertInterfaceNumbers(v *any, depth int) error {
	var err error
	switch v2 := (*v).(type) {
	case json.Number:
...
// values which are map[string]any or []any are recursively visited
func ConvertMapNumbers(m map[string]any, depth int) error {
	if depth > maxDepth {
...
// values which are map[string]any or []any are recursively visited
func ConvertSliceNumbers(s []any, depth int) error {
	if depth > maxDepth {
		return fmt.Errorf("exceeded max depth of %d", maxDepth)
	}
