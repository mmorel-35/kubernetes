...
var (
	...
	DefaultKubeletServingGetUsages = func(privateKey any) []certificates.KeyUsage {
	...
	DefaultKubeletClientGetUsages = func(privateKey any) []certificates.KeyUsage {
		switch privateKey.(type) {
		case *rsa.PrivateKey:
...
type Config struct {
	...
	GetUsages func(privateKey any) []certificates.KeyUsage
...
func NewManager(config *Config) (Manager, error) {
	...
	var getUsages func(any) []certificates.KeyUsage
	if config.GetUsages != nil {
		getUsages = config.GetUsages
	} else {
		getUsages = func(any) []certificates.KeyUsage { return config.Usages }
	}
	m := manager{
	...
	m.logf = config.Logf
	if m.logf == nil {
		m.logf = func(format string, args ...any) { klog.V(2).Infof(format, args...) }
	}
...
func (m *manager) generateCSR() (template *x509.CertificateRequest, csrPEM []byte, keyPEM []byte, key any, err error) {
